...
namespace ZentoAdv\ElasticCatalog\Console\Commands;
...
class SyncCategory2Els extends \Zento\Kernel\PackageManager\Console\Commands\Base
{
    ...
    public function handle()
    {
        ...
        switch($type) {
            case 'category':
                $syncer = new \ZentoAdv\ElasticCatalog\Processor\CategorySync();
                $syncer->sync();
                break;
            case 'product': 
                $syncer = new \ZentoAdv\ElasticCatalog\Processor\ProductSync();
                $syncer->sync();
                break;
            default:
